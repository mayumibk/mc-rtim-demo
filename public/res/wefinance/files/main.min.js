...
}catch(d){}}$(document).on("keyup","#gssSearchField",function(){var g=$("#gssSearchField").val().toString();
var a=[];
if($("#gssSearchField").val().length>3){var b=null;
var c="/bin/querybuilder.json?orderby=@jcr:score&orderby.sort=desc&p.limit=10&fulltext="+g;
...
d<e.hits.length;
d++){var f=e.hits[d];
if($.inArray(f.title,a)===-1){a.push(f.title)
}}}$("#gssSearchField").autocomplete({source:a})
});
(function(d){var b=d.event,a,c;
a=b.special.debouncedresize={setup:function(){d(this).on("resize",a.handler)
},teardown:function(){d(this).off("resize",a.handler)
